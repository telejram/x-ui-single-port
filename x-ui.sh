...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/telejram/x-ui-single-port/master/install_en.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/telejram/x-ui-single-port/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
